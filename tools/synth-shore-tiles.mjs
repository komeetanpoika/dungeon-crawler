...
// tiles with their one grass green recoloured to the bank material's
// dominant grain (the brown waterline stays) — ow_shore_<k> for beach sand
// (ow_sand_0) and ow_mud_<k> for a marsh pool's mud band (ow_dirt_0).
// shoreline() in
...
// the lightest non-green grain of a ground tile
const grain = f => {
  let light = null, hi = -Infinity
...
// ow_dirt_0.png is a 4-bit indexed PNG png-read.mjs cannot open: its
// dominant grain is copied here
const MUD = [234, 165, 108]
let n = 0
for (const [prefix, base] of [['ow_shore', 'ow_sand_0'], ['ow_mud', null]]) {
  const light = base ? grain(readPng(path.join(TILES, `${base}.png`))) : MUD
  for (const name of RIM_TILES[prefix.slice(3)]) {
    ...
    for (let i = 0; i < out.length; i += 4)
      if (out[i + 3] && isGreen(out[i], out[i + 1], out[i + 2])) out.set(light, i)
