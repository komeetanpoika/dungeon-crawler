import { weaponContents, makeRangedContents } from './entities.js'
...
export function rollChestLoot(depth, rng = Math.random) {
  ...
  const r = rng()
  if (r < 0.5) return { type: 'potion', amount: 4 }
  if (r < 0.75) return { type: 'weapon', ...weaponContents(pick(MELEE_POOLS[tier], rng)) }
